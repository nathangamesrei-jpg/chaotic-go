...
window.renderizarPassoEscolhaDeck = function() {
    ...
    tela.innerHTML = `
        ...
        <button id="btn-jogar-drome" style="display: none; margin-top: 20px; background: #4CAF50; color: #000; font-weight: bold; font-size: 14px; padding: 14px; border-radius: 8px; border: 2px solid #1b5e20; width: 85%; cursor: pointer; box-shadow: 0 5px 0 #1b5e20;" onclick="window.confirmarEntradaDrome()">
            ⚔️ ENTRAR NO DROME
        </button>
...
function carregarDecksParaEscolha(modo) {
    ...
    db.ref('jogadores/' + uid + '/decks').get().then(snapshot => {
        lista.innerHTML = "";
...
function verificarIrregularidadeDeck(deck, modo) {
    let problemas = [];
    if (!deck.ataques || deck.ataques.length !== 20) problemas.push(`Ataques: ${deck.ataques ? deck.ataques.length : 0}/20`);
    if (deck.ataques && deck.ataques.length > 0) {
        ...
        if (custoTotal !== 20) problemas.push(`Custo de Ataques: ${custoTotal}/20`);
    }
    if (!deck.locais || deck.locais.length !== 10) problemas.push(`Locais: ${deck.locais ? deck.locais.length : 0}/10`);
    let minCriaturas = modo === '6x6' ? 6 : modo === '3x3' ? 3 : 1;
    let criaturasReais = deck.criaturas ? deck.criaturas.filter(c => c !== null).length : 0;
    if (criaturasReais < minCriaturas) problemas.push(`Criaturas: ${criaturasReais}/${minCriaturas}`);
...
function renderizarFilaOnline() {
    ...
    tela.innerHTML = `
        ...
        <button class="btn-voltar-pequeno" style="margin-top: 30px; border-color: #e53935; color: #e53935;" onclick="window.cancelarFila()">Cancelar e Voltar</button>
    ...
    db.ref('fila_drome/' + window.estadoDrome.modo + '/' + uid).set({
        uid: uid,
    ...
    db.ref('fila_drome/' + window.estadoDrome.modo).on('value', snapshot => {
        ...
        if (lista.length >= 2) {
            ...
            if (p1[0] === uid) {
                ...
                db.ref('fila_drome/' + window.estadoDrome.modo + '/' + p1[0]).remove();
                db.ref('fila_drome/' + window.estadoDrome.modo + '/' + p2[0]).remove();
                db.ref('salas_drome/' + salaId).set({
                    p1: { uid: p1[0], nome: p1[1].nome, deck: p1[1].deck },
                    p2: { uid: p2[0], nome: p2[1].nome, deck: p2[1].deck },
                    modo: window.estadoDrome.modo, status: "iniciando"
                });
                window.cancelarFila();
...
window.cancelarFila = function() {
    ...
    let uid = localStorage.getItem("chaoticUID");
    let db = window._firebaseDB;
    if (db) db.ref('fila_drome/' + window.estadoDrome.modo + '/' + uid).remove();
    window.renderizarPassoEscolhaDeck();
};

// ==========================================
// PASSO 4C: Escolher Amigo
// ==========================================
function renderizarPassoEscolhaAmigo() {
    ...
    amigos.forEach((amigo, i) => {
        ...
        div.innerHTML = `
            ...
            <button style="background:#e53935;color:white;border:none;padding:8px 12px;border-radius:5px;font-weight:bold;cursor:pointer;font-size:10px;" onclick="window.desafiarAmigoDrome(${i})">DESAFIAR</button>
        `;
        lista.appendChild(div);
    });
}
...
window.desafiarAmigoDrome = function(index) {
    ...
    db.ref('salas_drome/' + salaId).set({
        ...
        modo: window.estadoDrome.modo, status: "aguardando"
    });
    db.ref('jogadores/' + amigo.uid + '/desafio_drome').set({
        de: uid, nome: window.perfilJogador.nome, salaId: salaId, modo: window.estadoDrome.modo
    });
...
function renderizarAguardandoAmigo(salaId, nomeAmigo) {
    ...
    let db = window._firebaseDB;
    if (!db) return;
    db.ref('salas_drome/' + salaId).on('value', snapshot => {
...
window.cancelarDesafioDrome = function(salaId) {
    let db = window._firebaseDB;
    if (db) {
        db.ref('salas_drome/' + salaId).update({ status: "cancelado" });
        setTimeout(() => db.ref('salas_drome/' + salaId).remove(), 2000);
    }
    renderizarPassoEscolhaAmigo();
};

// ==========================================
// ESCUTA DE DESAFIO RECEBIDO
// ==========================================
window.escutarDesafiosDrome = function() {
    let uid = localStorage.getItem("chaoticUID");
    let db = window._firebaseDB;
    if (!db) return;
    db.ref('jogadores/' + uid + '/desafio_drome').on('value', snapshot => {
        if (!snapshot.exists()) return;
        let desafio = snapshot.val();
        db.ref('jogadores/' + uid + '/desafio_drome').remove();
...
window.responderDesafioDrome = function(resposta, salaId, modo) {
    ...
    if (!db) return;
    if (resposta === 'recusar') {
        db.ref('salas_drome/' + salaId).update({ status: "recusado" });
    } else {
        ...
        setTimeout(() => {
            ...
            if (btn) btn.onclick = () => {
                db.ref('salas_drome/' + salaId).update({ status: "pronta" });
...
// Inicia escuta após o Firebase carregar
setTimeout(function() {
    if (window._firebaseDB && window.escutarDesafiosDrome) window.escutarDesafiosDrome();
}, 1500);
